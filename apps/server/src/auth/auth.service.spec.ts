...
describe('AuthService using createMock with DI', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        AuthService,
        {
          ...
          useValue: {
            register: jest.fn().mockResolvedValue(registerReturn),
          },
        },
        UsersService,
        JwtService,
  ...
  describe('register', () => {
    it('should return tokens & user id', async () => {
      const cats = await authService.register('anothertestuser1', 'test');
      expect(cats).toEqual(registerReturn);
    });
  });
